...
wget -O go-ipfs.tar.gz "https://dist.ipfs.io/go-ipfs/v0.4.13/go-ipfs_v${IPFS_VERSION}_linux-amd64.tar.gz" || exit 1;
...
# create key file
echo "/key/swarm/psk/1.0.0/" > /root/.ipfs/swarm.key
echo "/base16/" >> /root/.ipfs/swarm.key
echo "f120b091655ae1eb8178b7638fd33f3bf8d17219d7783eb66a447227e67e5dbc" >> /root/.ipfs/swarm.key
...
if [ "${NODEID}" == 0 ]; then
    ...
    for LOOPCOUNT in `seq 1 5`; do
        status=`curl -Is http://"${BOOTNODEIP}":5001/api/v0/id | head -n 1`
        echo "STATUS: ${status}"
        if [[ -z "$status" ]]; then
            echo "Sleeping..."
            sleep 5;
            ...
            IPFS_BOOTNODE_ID=`curl -s http://"${BOOTNODEIP}":5001/api/v0/id | awk -F"[,:}]" '{for(i=1;i<=NF;i++){if($i~/\042'ID'\042/){print $(i+1)}}}' | tr -d '"'`
